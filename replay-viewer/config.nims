...
# use-after-free fixed in bc_replay.nim (emscripten_exit_with_live_runtime)
...
# bc_error_len(). Aborting keeps linear memory intact, and the page reads
# bc_stage_ptr/len afterwards to report what the runtime was doing.
switch(
  "passL",
  (&"""
  -o {distDir / "bc_replay.js"}
  ...
  -s EXPORTED_FUNCTIONS=_main,_malloc,_free,_bc_load_replay,_bc_frame,_bc_input,_bc_packet_ptr,_bc_packet_len,_bc_mismatch_round,_bc_error_ptr,_bc_error_len,_bc_stage_ptr,_bc_stage_len,_bc_game_version_ptr,_bc_game_version_len,_bc_sim_sources_stamp_ptr,_bc_sim_sources_stamp_len
  """).replace("\n", " ")
)
