## Records `tests/fixtures/replay-bc24.json`, the committed bc24 fixture
## replay (design note §Tests item 17).
##
##   nim r --path:src tools/gen_bc24_fixture_replay.nim [out.json]
##
## It is a real recording, not a hand-written document: one scripted
## `gone-sharkin` vs `examplefuncsplayer24` game on `Yinyang`, seed 3,
...
## `tests/test_bc24_replay.nim` can prove a recording made at one
...
const
  ...
  DefaultOut = "tests/fixtures/replay-bc24.json"
  Map = "Yinyang"
  Seed = 3
  Rounds = 260
    ## Past the dam: the first 200 rounds are the setup phase, so a shorter
    ## fixture would record a game in which nothing can be attacked.
